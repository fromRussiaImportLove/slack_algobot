...
class GetTest(View):
    ...
    def post(self, request):
        ...
        """
        client.dialog_open(trigger_id = request.POST.get('trigger_id'), dialog = json.dumps(choose_test))
...
class GetHint(View):
    ...
    def post(self, request):
        client.dialog_open(trigger_id = request.POST.get('trigger_id'), dialog = json.dumps(choose_hint))
        return HttpResponse("Выбирай!", 200)
...
class Register(View):
    ...
    def post(self, request):
        client.dialog_open(trigger_id = request.POST.get('trigger_id'), dialog = json.dumps(dialog_plot))
        ...
        if interactive_action["type"] == "interactive_message":
            ...
            if callback_id == 'register_form':
                ...
                if User.objects.filter(username = name):
                    client.chat_postMessage(channel=channel, text='Маска, я тебя знаю!')
                else:
                    new_user = User.objects.create(username = name, email = email)
                    new_user.save()
                    client.chat_postMessage(channel=channel, text='Приятно познакомиться!')
            if callback_id == 'test_form':
                ...
                if Test.objects.filter(problem__title=task, number = test).exists():
                    ...
                    title  = 'Done!'
                    client.files_upload(channels = channel, title = title, file = result.test_file.path)
                else:
                    client.chat_postMessage(channel=channel, text='У меня такого нет :(')
            ...
            if button == 'click_me_123':
                ...
                client.chat_postMessage(text='Котику понравилось!', channel=channel)
                ...
                client.chat_postMessage(channel=channel, text='', attachments=attachments, blocks=blocks)
            elif button == 'click_me_test':
                print(interactive_action['trigger_id'])
                client.dialog_open(trigger_id = interactive_action['trigger_id'], dialog = json.dumps(choose_test))
            elif button == 'click_me_hint':
                client.dialog_open(trigger_id = interactive_action['trigger_id'], dialog = json.dumps(choose_hint))
    ...
    def post(self, request):
        ...
        if 'event' in body:
            ...
            if event_type == 'app_home_opened':
                ...
                client.chat_postMessage(channel = channel, text = text, blocks=blocks, icon_emoji=':chart_with_upwards_trend:')
